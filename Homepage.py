...
def init_data_fix():
    ...
    if not existing_products:
        ...
        team5.add_product("Blue Dress", 2, "S", "Blue", 750.0, 8, "images/dress.jpg")
        team5.add_product("Pink Dress", 2, "OneSize", "Pink", 950.0, 20, "images/dress2.jpg")
...
""", unsafe_allow_html=True)
    ...
    with col1:
        if st.button("  MEN COLLECTION ", use_container_width=True):
            st.session_state['selected_cat_id'] = 1
            go_to('category')
    with col2:
        if st.button(" WOMEN COLLECTION ", use_container_width=True):
...
def render_category():
    ...
    with st.expander(" Filter & Search ", expanded=False):
        ...
        #CATEGORY FILTER
        with c_filter1:
            filter_options = ["All", "Shirt", "Jeans", "Dress", "T-shirt", "Shoes"]
    ...
    if not final_products:
        st.info(f"There is no {selected_type} in this range. ")
    else:
...
def render_login():
    ...
    if st.button("Go to Login/Sign Up Page ➡", use_container_width=True, type="primary"):
...
def render_profile():
    ...
    # 1. تصحيح الاسم هنا: استخدام team5 بدلاً من db
    ...
    # تأكد أن العمود role موجود في الداتا بيز، لو مش موجود شيل السطر ده
...
def render_about():
    ...
    with col_contact1:
        ...
        st.markdown("📧 Email:** rawnaq@gmail.com")
